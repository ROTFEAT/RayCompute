...
2. **查找设计文档和计划（关键！）** — 用户很可能用其他工具生成过文档，按优先级搜索：
   - `~/.gstack/projects/` — gstack `/office-hours` 生成的设计文档（`*-design-*.md`）
   - `~/.claude/plans/` — superpowers `/write-plan` 生成的实施计划
   - 当前项目的 `tasks/plan-*.md` — 之前 `/ray-new` 生成的计划文件
   - 当前项目的 `*.md` 文件（排除 README/CLAUDE.md）— 任何含"设计"、"计划"、"需求"、"参数"、"数据"关键词的文档
   - `~/.claude/todos/` — 待办事项中可能有相关需求
   - 如果找到文档，**读取最近的那个**——里面的需求描述、数据结构、计算逻辑、参数空间直接用于生成 Ray 脚本
